				...
				>
					...
					<div class="p-2 space-y-1">
						...
						<p class="text-xs {isDark ? 'text-gray-300' : 'text-gray-700'}">
							Down: {incident.down ?? 0} | Empty: {incident.empty ?? 0}
						</p>
						{#if (incident.upside_down ?? 0) > 0 || (incident.broken_glass ?? 0) > 0}
							<p class="text-xs {isDark ? 'text-gray-500' : 'text-gray-400'}">
								Upside: {incident.upside_down ?? 0} | Broken: {incident.broken_glass ?? 0}
							</p>
						{/if}
					</div>
				</div>
