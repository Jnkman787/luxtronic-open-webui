				...
				>
					...
					<div class="p-2 space-y-1">
						...
						<p class="text-xs {isDark ? 'text-gray-300' : 'text-gray-700'}">
							Down: {incident.down ?? 0} | Ring: {incident.ring ?? 0} | Empty: {incident.empty_mat ?? 0}
						</p>
						<p class="text-xs {isDark ? 'text-gray-300' : 'text-gray-700'}">
							Gate: {incident.gate ?? 0}
						</p>
						<p class="text-xs {isDark ? 'text-gray-500' : 'text-gray-400'}">
							Conf: {formatConf(incident.down_conf)} | {formatConf(incident.ring_conf)} |
							{formatConf(incident.empty_mat_conf)} | {formatConf(incident.gate_conf)}
						</p>
					</div>
				</div>
			{/each}
