...
- Dashboard APIs live under `/api/v1/dashboard` and proxy requests to rag-platform.
- Admins see all dashboards; non-admins are restricted by tenant identifiers and `tenant_group_name`.
- Tenant configs can include `factories`, `dashboard_views`, and `time_ranges` to drive modular UI.
